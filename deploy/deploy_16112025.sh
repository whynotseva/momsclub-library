#!/bin/bash
#
# Скрипт деплоя версии от 16.11.2025
# Создает бэкап, загружает файлы, запускает миграцию и перезапускает сервис
#
# Использование на сервере:
# cd /root/home/momsclub
# chmod +x deploy/deploy_16112025.sh
# sudo ./deploy/deploy_16112025.sh

set -e

# Цвета
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

BACKUP_DATE="16112025"
BACKUP_DIR="momsclub${BACKUP_DATE}"
CURRENT_DIR="/root/home/momsclub"
BACKUP_PATH="/root/home/${BACKUP_DIR}"
SERVICE_NAME="telegram-bot"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}ДЕПЛОЙ ВЕРСИИ ОТ 16.11.2025${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""

# Проверка root
if [ "$EUID" -ne 0 ]; then 
    echo -e "${RED}Ошибка: запустите с sudo${NC}"
    exit 1
fi

cd "$CURRENT_DIR"

# Шаг 1: Создание бэкапа
echo -e "${YELLOW}Шаг 1: Создание бэкапа текущей версии${NC}"
echo "────────────────────────────────────────────"
echo "Бэкап будет создан в: $BACKUP_PATH"
echo ""

# Проверка существования бэкапа
if [ -d "$BACKUP_PATH" ]; then
    echo -e "${YELLOW}⚠ Бэкап $BACKUP_PATH уже существует${NC}"
    read -p "Перезаписать? (yes/no): " overwrite
    if [ "$overwrite" != "yes" ]; then
        echo "Отменено"
        exit 0
    fi
    rm -rf "$BACKUP_PATH"
fi

# Остановка сервиса
echo "Остановка сервиса..."
if systemctl is-active --quiet $SERVICE_NAME 2>/dev/null; then
    systemctl stop $SERVICE_NAME 2>/dev/null || true
    sleep 2
    echo "✓ Сервис остановлен"
else
    echo "⚠ Сервис не был запущен"
fi

# Создание директории бэкапа
mkdir -p "$BACKUP_PATH"
echo "✓ Создана директория $BACKUP_PATH"

# Копирование файлов
echo "Копирование файлов проекта..."
rsync -av \
    --exclude='venv' \
    --exclude='__pycache__' \
    --exclude='*.pyc' \
    --exclude='*.log' \
    --exclude='*.db.backup*' \
    --exclude='.git' \
    --exclude='.env' \
    "$CURRENT_DIR/" "$BACKUP_PATH/" > /dev/null 2>&1

# Сохранение .env и БД
if [ -f "$CURRENT_DIR/.env" ]; then
    cp "$CURRENT_DIR/.env" "$BACKUP_PATH/.env.backup"
    echo "✓ Сохранен .env файл"
fi

if [ -f "$CURRENT_DIR/momsclub.db" ]; then
    cp "$CURRENT_DIR/momsclub.db" "$BACKUP_PATH/momsclub.db"
    echo "✓ Скопирована база данных momsclub.db"
fi

echo -e "${GREEN}✓ Бэкап создан успешно${NC}"
echo ""

# Шаг 2: Проверка новых файлов
echo -e "${YELLOW}Шаг 2: Проверка новых файлов${NC}"
echo "────────────────────────────────────────────"

# Проверяем наличие обновленных файлов
REQUIRED_FILES=(
    "handlers/admin/users.py"
    "database/migrations/create_admin_action_logs_table.py"
)

MISSING_FILES=()
for file in "${REQUIRED_FILES[@]}"; do
    if [ ! -f "$CURRENT_DIR/$file" ]; then
        MISSING_FILES+=("$file")
    fi
done

if [ ${#MISSING_FILES[@]} -gt 0 ]; then
    echo -e "${YELLOW}⚠ Не найдены следующие файлы:${NC}"
    for file in "${MISSING_FILES[@]}"; do
        echo "  - $file"
    done
    echo ""
    echo "Загрузите новые файлы на сервер перед продолжением"
    echo "Используйте scp или другой метод для загрузки"
    echo ""
    read -p "Продолжить деплой? (yes/no): " continue_deploy
    if [ "$continue_deploy" != "yes" ]; then
        echo "Деплой отменен. Для отката: ./deploy/rollback.sh $BACKUP_DATE"
        exit 0
    fi
else
    echo -e "${GREEN}✓ Все необходимые файлы найдены${NC}"
fi

echo ""

# Шаг 3: Обновление зависимостей
echo -e "${YELLOW}Шаг 3: Обновление зависимостей${NC}"
echo "────────────────────────────────────────────"

if [ -f "requirements.txt" ] && [ -d "venv" ]; then
    echo "Обновление зависимостей..."
    source venv/bin/activate
    pip install -r requirements.txt --quiet --upgrade 2>&1 | grep -v "already satisfied" || true
    echo -e "${GREEN}✓ Зависимости обновлены${NC}"
else
    echo "⚠ venv не найден, пропускаю обновление зависимостей"
fi

echo ""

# Шаг 4: Проверка синтаксиса
echo -e "${YELLOW}Шаг 4: Проверка синтаксиса Python${NC}"
echo "────────────────────────────────────────────"

if command -v python3 &> /dev/null; then
    echo "Проверка основных файлов..."
    python3 -m py_compile bot.py handlers/admin/users.py 2>&1 | head -10 || {
        echo -e "${RED}Ошибка: найдены синтаксические ошибки${NC}"
        echo "Для отката используйте: ./deploy/rollback.sh $BACKUP_DATE"
        exit 1
    }
    echo -e "${GREEN}✓ Синтаксис корректен${NC}"
else
    echo "⚠ python3 не найден, пропускаю проверку синтаксиса"
fi

echo ""

# Шаг 5: Миграция базы данных
echo -e "${YELLOW}Шаг 5: Выполнение миграции базы данных${NC}"
echo "────────────────────────────────────────────"
echo -e "${YELLOW}ВНИМАНИЕ: Это создаст таблицу admin_action_logs${NC}"
echo ""

# Проверяем, существует ли уже таблица
if [ -f "momsclub.db" ]; then
    if python3 -c "import sqlite3; conn = sqlite3.connect('momsclub.db'); cursor = conn.cursor(); cursor.execute(\"SELECT name FROM sqlite_master WHERE type='table' AND name='admin_action_logs'\"); print('exists' if cursor.fetchone() else 'not_exists')" 2>/dev/null | grep -q "exists"; then
        echo "⚠ Таблица admin_action_logs уже существует"
        read -p "Выполнить миграцию повторно? (yes/no): " rerun_migration
        if [ "$rerun_migration" != "yes" ]; then
            echo "Миграция пропущена"
            SKIP_MIGRATION=true
        fi
    fi
fi

if [ "${SKIP_MIGRATION:-false}" != "true" ]; then
    if [ -f "database/migrations/create_admin_action_logs_table.py" ]; then
        echo "Запуск миграции create_admin_action_logs_table.py..."
        if [ -d "venv" ]; then
            source venv/bin/activate
        fi
        python3 database/migrations/create_admin_action_logs_table.py
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}✓ Миграция выполнена успешно${NC}"
        else
            echo -e "${RED}Ошибка при выполнении миграции!${NC}"
            echo "Для отката используйте: ./deploy/rollback.sh $BACKUP_DATE"
            exit 1
        fi
    else
        echo -e "${YELLOW}⚠ Файл миграции не найден, пропускаю${NC}"
    fi
fi

echo ""

# Шаг 6: Запуск сервиса
echo -e "${YELLOW}Шаг 6: Запуск сервиса${NC}"
echo "────────────────────────────────────────────"

if systemctl start $SERVICE_NAME 2>/dev/null; then
    echo -e "${GREEN}✓ Сервис $SERVICE_NAME запущен${NC}"
else
    echo -e "${RED}Ошибка: не удалось запустить сервис${NC}"
    echo "Проверьте конфигурацию: systemctl status $SERVICE_NAME"
    echo "Для отката используйте: ./deploy/rollback.sh $BACKUP_DATE"
    exit 1
fi

# Проверка статуса
sleep 3
if systemctl is-active --quiet $SERVICE_NAME 2>/dev/null; then
    echo -e "${GREEN}✓ Сервис работает${NC}"
else
    echo -e "${YELLOW}⚠ Сервис не запустился, проверьте логи${NC}"
    echo "Проверьте: journalctl -u $SERVICE_NAME -f --lines=50"
fi

echo ""

# Финальный отчёт
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}ДЕПЛОЙ ЗАВЕРШЕН${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo "Бэкап сохранён в: $BACKUP_PATH"
echo "Для отката используйте: ./deploy/rollback.sh $BACKUP_DATE"
echo ""
echo "Проверьте работу:"
echo "  systemctl status $SERVICE_NAME"
echo "  journalctl -u $SERVICE_NAME -f --lines=50"
echo ""

