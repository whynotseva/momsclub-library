...
export function QuoteOfDay({ author = 'Полина' }: QuoteOfDayProps) {
  ...
  return (
    <div className="mb-8 bg-gradient-to-r from-[#F5E6D3]/50 to-[#ECD9C8]/30 dark:from-[#2A2A2A]/50 dark:to-[#1E1E1E]/30 rounded-2xl p-4 lg:p-6 border border-[#E8D4BA]/30 dark:border-[#3D3D3D]">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start space-x-3 flex-1">
          ...
          >
            <p className="text-[#2D2A26] dark:text-[#E5E5E5] font-medium italic text-sm lg:text-base">
        ...
        <button
          ...
          className={`flex items-center space-x-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 flex-shrink-0 ${
            isAnimating
              ? 'bg-[#E8D4BA]/50 dark:bg-[#3D3D3D]/50 text-[#8B8279] dark:text-[#707070] cursor-not-allowed'
              : 'bg-white/70 dark:bg-[#2A2A2A]/70 hover:bg-white dark:hover:bg-[#2A2A2A] text-[#B08968] hover:shadow-md border border-[#E8D4BA]/50 dark:border-[#3D3D3D]'
          }`}
        >
