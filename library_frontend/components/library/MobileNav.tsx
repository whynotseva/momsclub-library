...
export function MobileNav({ activePage = 'library', isPWA = false, isVisible = true }: MobileNavProps) {
  ...
  return (
    ...
    >
      <div
        className="flex items-center justify-around rounded-2xl px-2 py-3 shadow-2xl border border-white/50 dark:border-[#3D3D3D]"
        style={{ background: resolvedTheme === 'dark' ? 'rgba(30,30,30,0.85)' : 'rgba(255,255,255,0.45)', backdropFilter: 'blur(24px) saturate(180%)' }}
      >
        {isPWA && (
          <button
            onClick={() => window.location.reload()}
            className="p-2.5 rounded-xl text-[#B08968] hover:bg-[#F5E6D3] dark:hover:bg-[#2A2A2A] transition-colors"
            title="Обновить"
          >
            <svg
              width="18"
